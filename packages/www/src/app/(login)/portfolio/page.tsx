...
export default function PortfolioPage() {
  ...
  const handleSend = async () => {
    ...
    try {
      ...
      const walletAccount = privateKeyToAccount(privateKeyHex);
      ...
      console.log(`Sending ${sendAmount} ETH (${amountToSendWei} wei) from ${walletAccount.address} to ${destinationAddress}`);
  ...
  return (
    <div className="flex flex-col items-center justify-center w-full min-h-screen p-4 md:p-8">
      <main className="w-full max-w-4xl flex flex-col items-center gap-4">
        ...
        {/* Show loading or alternative message if required data isn't available yet */}
        {!ethAddress && loggedInCredentialInfo && (
           <p>Loading Ethereum details...</p>
        )}
